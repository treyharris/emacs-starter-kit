...
#+begin_src emacs-lisp 
  ...
  (defun ash-opener (char)
    "Return the opening equivalent of CHAR, as a string."
    (if (memq char '(?\( ?\[ ?<))
        (char-to-string char)
      (ash-pair-get-other char)))
  
  (defun ash-pair-get-other (char)
    (or (cdr (assoc char ash-clear-pairs)) (char-to-string char)))
  
  (defun ash-clear (char)
    (interactive "*cClear within: ")
    (let* ((opener (ash-opener char))
           (closer (ash-pair-get-other (string-to-char opener)))
           (begin (search-backward-regexp (format "%s\\|%s" (regexp-quote opener)
                                                 (regexp-quote closer)))))
      (forward-char)
      (search-forward (ash-pair-get-other (string-to-char (match-string 0))))
